...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->auth_override === FALSE && !($this->config->item('rest_enable_keys') && $this->_allow === TRUE))
        {
            ...
            //Modified by Sudhan Raj//
            //Checks for the presence of 4 HTTP Headers//
            $this->secret_key		= 'HomageRockz!';
            $headers = $this->input->request_headers();

            $this->UDID			= isset($headers['X-Client-Identifier']) ? $headers['X-Client-Identifier'] : false;
            $this->platform 		= isset($headers['X-Client-Platform']) ? $headers['X-Client-Platform'] : false;
            $this->client_version           = isset($headers['X-Client-Version']) ? $headers['X-Client-Version'] : false;

            if (!$this->UDID || !$this->platform || !$this->client_version)
                    $this->response(array('alert' => $this->alertData('Missing Parameters'), 'headers' => $headers), 400);

            if ($this->platform !== 'iPhone' && $this->platform !== 'Android' && $this->platform !== 'Blackberry' && $this->platform !== 'Windows' && $this->platform !== 'Development')
                    $this->response(array('alert' => $this->alertData('Platform not recognised')), 400);

            $this->load->model('serve/app_auth', 'app_auth');

            $access_token                   = '';
            if ($this->query('access_token'))
                $access_token               = $this->query('access_token');
            else if ($this->get('access_token'))
                $access_token               = $this->get('access_token');
            else if ($this->post('access_token'))
                $access_token               = $this->post('access_token');
            else if ($this->put('access_token'))
                $access_token               = $this->put('access_token');

            if ($access_token) {
                $this->auth_user            = $this->app_auth->validateToken($access_token);
            }

            $this->limit                    = $this->get('limit') ? $this->get('limit') : 10;
            $this->offset                   = $this->get('offset') ? $this->get('offset') : 0;
            $this->lang->load('homage_api', 'english');
            //End of Sudhan Raj Mod//
    }

    public function alertData($title = '', $message = '') {
            return array('title' => $title, 'message' => $message);
    }
    
    public function isUserAuth() {
        if (is_array($this->auth_user) && $this->auth_user['id'])
            return true;
        
        $this->response(['alert' => $this->alertData($this->lang->line('resp_session_expired'))], 403);
    }
    
    public function sendAlert($message = false, $status = 400) {
        if ($message === false)
            $message = $this->lang->line('resp_system_error');
        
        $this->response(['alert' => $this->alertData($message)], $status);
    }
